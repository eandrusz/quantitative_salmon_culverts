## sampling posteriors to smash together
## RPK and EAA 
## Sept 1, 2022
...
ggplot(mergedc, aes(x=time, y=log10(meandnaconc))) +
  geom_point() +
  geom_point(aes(x=time, y=log10(meantotdna)), color="blue") +
  facet_grid(~station ~creek ~species) 

simple <- mergedc %>% 
  select(c(bottle, time, creek, station, bio, species, meanpropreads, meancutdna, meantotdna, meandnaconc))
