...
#### Make Stan objects
  ...
  stan_data <- list(
    ...
    # 
    # up_idx =  as.array(seq(2, nrow(p_samp_all %>% filter(tech_rep == 1)), 2)), #vector of length (N_obs_samp_small/2), giving index of upstream stations
    # down_idx =  as.array(seq(1, nrow(p_samp_all %>% filter(tech_rep == 1)), 2)), #vector of length (N_obs_samp_small/2), giving index of downstream stations
