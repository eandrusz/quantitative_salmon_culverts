...
format_metabarcoding_data <- function(input_metabarcoding_RDS, input_mock_comm_RDS){
  ...
  Observation <- readRDS(input_metabarcoding_RDS) %>% 
    filter(species != "") %>% 
    filter(time != "NA") %>% 
    filter(Nreads > 0) %>% # ignore anything that was never observed
    filter(tech == 1) %>% # ignore any tech replication, for the moment, by only taking a single replicate per sample
    unite(c(time, creek, site, biol), col = "Sample", sep = ".", remove = F)

  Mock <- readRDS(input_mock_comm_RDS) %>% 
    ...
    # filter(InputType == "Tissue") %>%  #omit once we select best mock to use
    filter(start_prop > 0 & Nreads > 0) %>% #omit things that are absent from the mocks
  ...
  Mock <- Mock %>% 
    left_join(sp_list) %>% 
    mutate(site = match(site, unique(site)),
           speciesname = species,
           species = species_idx) %>% 
  ...
  makeDesign <- function(obs, #obs is a named list with elements Observation, Mock, N_pcr_mock, sp_list
    ...
    stan_data <- list(
      ...
      sample_vector = p_samp_all$site,
      mock_data   = mock %>% dplyr::select(contains("sp")),
      sp_list = metabarcoding_data$sp_list,
...
QM_likelihood <- function(stanmodelname, stan_metabarcoding_data){
    ...
    stanOpt <- optimizing(M, data=stan_metabarcoding_data, iter=30000,draws=0,
                          verbose=T,  #TODO make reasonable init function to avoid risk of stochastic failure to converge
                          tol_param=1e-15,
                          algorithm="LBFGS")
    ...
    ML_a <- data.frame("alpha_est" = ML_a, 
               "species" = metabarcoding_data$sp_list$species)
...
##example
  ...
  #   separate(col = sample, into = c("time", "creek", "site", "biol"), remove = FALSE) %>% 
  #   filter(value > 0.001) %>% 
