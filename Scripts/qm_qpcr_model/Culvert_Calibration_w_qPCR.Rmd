...
stan_qPCR_data <- list(
  ...
  SampleNames = unique(qPCRdata$Sample),
  ...
  stdCurvePrior_intercept = c(40, .5), #normal distr, mean and sd
...
#fit model
      ...
      #keep only relevant qPCR samples for comparison
      envir_qPCR <- results_qPCR %>% filter(Sample %in% keepSamples) %>% dplyr::select(plateSample, mean_corrected, ci25_corrected, ci75_corrected) %>% distinct()
      
      #scale by qPCR values to estimate absolute concentration 
      totalDNA <- envir_qPCR$mean_corrected / MLest[,RefSpecies]
...
ppq <- 10^extract(qMod, par = "envir_concentration")$envir_concentration[,which((qPCRdata %>% filter(Type != "STANDARD") %>% pull(Sample) %>% unique()) %in% keepSamples)]
 #get posterior samples for absolute concentration of reference species

#apply dilution
dilutionFactor <- qPCRdata %>% 
  filter(Sample %in% keepSamples) %>% 
  dplyr::select(Sample, dilution) %>% 
  distinct() %>% 
  pull(dilution)
ppq <- ppq * dilutionFactor
