...
qPCRdata <- qPCRdata %>%
  ...
  mutate(sdCt = sd(Ct)) %>% 
  filter(sdCt < 1.2) #really bad triplicates
...
stan_qPCR_data <- list(
  ...
  stdCurvePrior_intercept = c(39, 3), #normal distr, mean and sd ; NOTE make hierarch across plates
  stdCurvePrior_slope = c(-3, .5) #normal distr, mean and sd ; NOTE make hierarch across plates
)
...
results_qPCR <- qPCRdata %>%
  ...
  left_join(data.frame(
    qPCRdata %>% filter(Task != "STANDARD") %>% dplyr::select(Sample) %>% distinct()) %>% 
    ...
    mutate(mean_corrected = bayes_est ,
         ci25_corrected = bayes_25pct ,
         ci75_corrected = bayes_75pct ) %>% 
