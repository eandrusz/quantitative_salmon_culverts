...
ggplot(mergedb, aes(x=time, y=log(meantotdna))) +
  geom_point() +
  geom_point(aes(x=time, y=log(meancutdna), color="blue")) +
  facet_grid(~station ~creek) 

simple <- mergedb %>% 
  select(c(bottle, meanpropreads, meancutdna, meantotdna))]

#write.rds(simple, here("Output","salmonids_abs_abundance_bio_posterior.RDS")
