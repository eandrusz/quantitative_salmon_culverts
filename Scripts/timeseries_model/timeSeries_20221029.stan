...
parameters {
  ...
  real gamma_unk[NconstructionTimepoints, Nspecies, Nconstruction-1];
...
transformed parameters {
  ...
  for (i in 1:N_unobserved){
    ...
    //define gamma array as 0.0, and then slot in param values where construction is present
      gamma = rep_array(0.0, Ntime-1, Nspecies, Nconstruction); 
      
      //then fill in the rest of the array w param values
       for (t in 1:NconstructionTimepoints){
          for (j in 1:Nspecies){
              gamma[(t + MinconstructionTimepoint - 2),j,2] = gamma_unk[t,j,1];
            }}

       //define gamma to be 0 for all species/time where construction is absent
       // gamma[,,1] = rep_array(0.0, Ntime-1, Nspecies); //define gamma to be 0 for all species/time where construction is absent
       // gamma[1:(MinconstructionTimepoint-2),,2] = rep_array(0.0, (Ntime-1)-NconstructionTimepoints, Nspecies);
