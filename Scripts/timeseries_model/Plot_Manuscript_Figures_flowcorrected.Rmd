...
####
stanMod <- readRDS(here("Scripts","timeseries_model","20221129_modelFit_flowcorrected.RDS"))
...
multispeciesTrendsplot <- ggplot() +  #treating meandnaconc as observations
   ...
   # ggsave(multispeciesTrendsplot, file = here("Output/Figures/20221129_multispeciesTrends_flowcorrected.png"), width=9, height=7)
...
(construction_effect <- gamma_est %>% 
  ...
  left_join(f %>% dplyr::select(species_idx, species) %>% distinct()) %>% 
  mutate(jitter_time = time_idx + (jitter)) %>% 
  left_join(resOut %>% filter(creek_idx == 2, station_idx == 1) %>% dplyr::select(time_idx, species_idx, mean_est)) %>% distinct() %>% 
  ...
  theme_bw() +
  labs( x="Date (YY-MM)", y= "Percent Change Due to Culvert Removal", title="Effect of Construction in Padden") +
  scale_color_discrete(type = colorPal)) 

 # ggsave(construction_effect, file = here("Output/Figures/20221129_Construction_effect.png"))
...
delta <- f %>% select(newtime, time_idx) %>%
  ...
  (culvert_effect <-  delta %>% 
      ...
      ggplot(aes(x = newtime, y = delta_mean_pct)) +
      geom_boxplot(fill = "grey") +
      geom_hline(yintercept=0, color="grey") +
      xlab("Date (YY-MM)") + ylab("Mean difference upstream - downstream \n(% of downstream concentration)") +
      theme_bw() + 
      scale_x_discrete(guide = guide_axis(angle = -45))
      )  #here, just taking means, and summarizing across species and creeks w boxplot
  
 # ggsave(culvert_effect, file = here("Output/Figures/20221129_culvert_effect_flowcorrected.png"))
  ...
  # ggsave(culvert_effect_creek_species, file = here("Output/SupplementalFigures/20221129_culvert_effect_creek_species_flowcorrected.png"))
