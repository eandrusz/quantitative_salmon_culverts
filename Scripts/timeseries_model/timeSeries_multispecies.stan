...
model {
  ...
  for (i in 1:Nobs){
    y_logeDNA[i] ~ normal(mu[time_idx[i], station_idx[i], species_idx[i], creek_idx[i]], 
                          sigma_dna[species_idx[i]]); //sigma_dna[creek_idx[i]]
  }

for (d in 1:Nstations){
  for (i in 1:Ncreek){
    for (j in 1:Nspecies){
      eta[,d,j,i] ~ normal(0, sigma_eta[species_idx[j]]);
...
generated quantities{
  // vector[Ncreek] ysim[Ntime, Nstations, Nrep];
  // // int timesim[Ntime];
  // // int stationsim[Nstations];
  // // int creeksim[Ncreek];
  // 
  // for (t in 1:Ntime){
  //     for (i in 1:Ncreek){
  //       for (d in 1:Nstations){
  //         for (j in 1:Nrep){
  //           ysim[time_idx[t], station_idx[d], j, creek_idx[i]] = normal_rng(mu[time_idx[t], station_idx[d], creek_idx[i]], sigma_dna);    
  //         }
  //       }
  //     }
  //   } 
