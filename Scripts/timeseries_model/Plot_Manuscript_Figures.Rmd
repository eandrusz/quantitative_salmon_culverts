...
```{r, echo = F, warning = F, message = F}
    (multispeciesTrends <- f %>%
      ...
      ggplot(aes(x = month, y = log(meandnaconc), color = station)) +  #treating meandnaconc as observations
          geom_point(alpha = .2) +
          geom_point(aes(x = month, y = mean_est, color = station), size = 1.5, alpha = .5) + #mean estimates of mu
          geom_smooth(aes(x = month, y = mean_est, color = station), se = F, size = 1, alpha = .5, span = .2) +
          geom_segment(aes(x = month, xend = month, y = pp_025, yend = pp_975, color = station), size = .3,  alpha = .2) + #full error model creates CI
    ...
    #ggsave(multispeciesTrends, file = here("Output/Figures/multispeciesTrends.png"))
...
delta <- f %>% select(newtime, time_idx) %>%
    ...
    mutate(delta_mean_pct = delta_mean/(mean_est-delta_mean),  #scale to make percent change
           delta_025_pct = delta_025/(mean_est-delta_mean),
           delta_975_pct = delta_975/(mean_est-delta_mean)) %>% 
