...
ggsave(here("Output","Figures", "modeled_cut_qpcr_updown.png"))


####################################################################
# qPCR: cutthroat modeled concentration DIVIDED BY FLOW
####################################################################
flowmerge <- monthlyflow %>% 
...
cut_flow_corrected %>% 
  ggplot(aes(x = newtime, y = log10(mean_concentration_est))) +
  geom_point() +
  geom_point(aes(x = newtime, y = log10(mean_conc_flow_correct), color="blue")) +
  #geom_segment(aes(x = newtime, xend = newtime, y = log10(ci25_concentration_est), yend = log10(ci75_concentration_est))) +
  facet_grid(~creek~station) +
  labs(x="Date (YY-MM)", y= "Log10 copies/L water", title = "Cutthroat Trout") + 
  theme_bw() + 
...
ggsave(here("Output","Figures", "modeled_cut_qpcr_updown_flowcorrected.png"))
